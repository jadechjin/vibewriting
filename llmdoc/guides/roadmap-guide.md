...
| Phase 2 | ~245-301 | 数据模型 + 处理管线（已完成） |
| Phase 3 | ~305-351 | 文献整合工作流（已完成） |
| Phase 4 | ~355-421 | 单 Agent 草稿撰写（已完成） |
| Phase 5 | ~425-478 | 多 Agent 编排（已完成） |
| Phase 6 | ~482-547 | 编译 + 质量保证（**已完成**） |
| Phase 7 | ~551-583 | 端到端集成（**已完成**） |
...
### Approval Gates（5 个断点）

| 断点 | 触发条件 | 用户操作 |
|------|---------|---------|
| Phase 2 -> 4 | 图表 + asset_manifest 完成 | `/approve` 或调整资产 |
| Phase 3 -> 4 | 证据卡 + BibTeX 完成 | `/approve` 或补充文献 |
| Phase 4 -> 5 | 单 Agent 草稿 + 门禁通过 | `/approve` 或手动修订（**已完成**） |
| Phase 5 -> 6 | 多 Agent 合并完成 | `/approve` 或重写章节（**已完成，可启动**） |
| Phase 6 -> 7 | Phase 6 审查报告通过 | `/approve` 或处理遗留问题（**已完成**） |
